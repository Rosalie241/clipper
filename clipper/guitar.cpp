...
enum
{
    ...
    PS4_GIBSONSG_DONGLE_PRODUCT_ID = 0x5500,

    PS5_RIFFMASTER_VENDOR_ID  = 0x0E6F,
    PS5_RIFFMASTER_PRODUCT_ID = 0x0249,

    PS5_GIBSONSG_VENDOR_ID         = 0x3651,
    PS5_GIBSONSG_PRODUCT_ID        = 0x1600,
    PS5_GIBSONSG_DONGLE_PRODUCT_ID = 0x5600,
};

enum
{
    PS4_BTN_MASK_FRET_1 = 0b00000001,
    PS4_BTN_MASK_FRET_2 = 0b00000010,
    PS4_BTN_MASK_FRET_3 = 0b00000100,
    PS4_BTN_MASK_FRET_4 = 0b00001000,
    PS4_BTN_MASK_FRET_5 = 0b00010000,

    PS4_BTN_MASK_DPAD   = 0b00001111,

    PS4_BTN_MASK_STICK  = 0b01000000,
    PS4_BTN_MASK_START  = 0b00100000,
    PS4_BTN_MASK_SELECT = 0b00010000,
    PS4_BTN_MASK_HOME   = 0b00000001,
};
...
enum
{
    ...
    PS4_BUF_TILT        = 45,
    PS4_BUF_FRETS       = 46,
    PS4_BUF_LOWER_FRETS = 47,
};

enum
{
    PS5_BTN_MASK_FRET_1 = 0b00000001,
    PS5_BTN_MASK_FRET_2 = 0b00000010,
    PS5_BTN_MASK_FRET_3 = 0b00000100,
    PS5_BTN_MASK_FRET_4 = 0b00001000,
    PS5_BTN_MASK_FRET_5 = 0b00010000,

    PS5_BTN_MASK_DPAD = 0b00001111,

    PS5_BTN_MASK_STICK  = 0b01000000,
    PS5_BTN_MASK_START  = 0b00100000,
    PS5_BTN_MASK_SELECT = 0b00010000,
    PS5_BTN_MASK_HOME   = 0b00000001,
};
...
enum
{
    ...
    PS5_BUF_DPAD        = 8,
    PS5_BUF_SYSTEM_BTNS = 9,
    PS5_BUF_PS_BTN      = 10,

    PS5_BUF_WHAMMY      = 41,
    PS5_BUF_TILT        = 42,
    PS5_BUF_FRETS       = 43,
    PS5_BUF_LOWER_FRETS = 44,
};
...
{
    for (const GuitarDevice& guitarDevice : l_SupportedPS4GuitarDevices)
    {
            ...
            deviceInfo->vendor_id == guitarDevice.VendorId)
        {
            ...
            deviceType = DeviceType::PS4Guitar;
            return true;
        }
    }
...
{
    for (const GuitarDevice& guitarDevice : l_SupportedPS5GuitarDevices)
    {
        if (deviceInfo->product_id == guitarDevice.ProductId &&
            deviceInfo->vendor_id == guitarDevice.VendorId)
        {
            deviceName = guitarDevice.ProductName;
            deviceType = DeviceType::PS5Guitar;
            return true;
        }
    }
...
void PS4GuitarPollInputThread(PVIGEM_CLIENT client, hid_device* device, std::string devicePath, GuitarDeviceConfiguration configuration)
{
    ...
    while (IsRunning)
    {
        ...
        buffer[PS4_BUF_DPAD] &= PS4_BTN_MASK_DPAD;
        // lower frets buffer matches the frets buffer
        buffer[PS4_BUF_FRETS] |= buffer[PS4_BUF_LOWER_FRETS];

        virtual_report.wButtons = ((buffer[PS4_BUF_FRETS] & PS4_BTN_MASK_FRET_1) << 12) |
                                  ((buffer[PS4_BUF_FRETS] & PS4_BTN_MASK_FRET_2) << 12) |
                                  ((buffer[PS4_BUF_FRETS] & PS4_BTN_MASK_FRET_3) << 13) |
                                  ((buffer[PS4_BUF_FRETS] & PS4_BTN_MASK_FRET_4) << 11) |
                                  ((buffer[PS4_BUF_FRETS] & PS4_BTN_MASK_FRET_5) << 4) |
                                  ((buffer[PS4_BUF_SYSTEM_BTNS] & PS4_BTN_MASK_STICK)) |
                                  ((buffer[PS4_BUF_SYSTEM_BTNS] & PS4_BTN_MASK_START) >> 1) |
                                  ((buffer[PS4_BUF_SYSTEM_BTNS] & PS4_BTN_MASK_SELECT) << 1) |
                                  ((buffer[PS4_BUF_PS_BTN] & PS4_BTN_MASK_HOME) << 10) |
                                  ((dpad_values[buffer[PS4_BUF_DPAD]]));

        virtual_report.sThumbRX = ((buffer[PS4_BUF_WHAMMY] * 255) - 32767);

        // account for deadzone
        const SHORT tilt_value = static_cast<SHORT>(min((buffer[PS4_BUF_TILT] * (128 * tilt_sensitivity)), MAXSHORT));
        virtual_report.sThumbRY = (tilt_value < tilt_deadzone ? 0 : tilt_value);

        virtual_report.sThumbLX = ((buffer[PS4_BUF_STICK_X] * 255) - 32767);
        virtual_report.sThumbLY = ((buffer[PS4_BUF_STICK_Y] * 255) - 32767);

        if (configuration.HasPickupSwitch)
        {
            virtual_report.bLeftTrigger = (buffer[PS4_BUF_PICKUP] >= sizeof(pickup_values))
                                            ? 0
                                            : pickup_values[buffer[PS4_BUF_PICKUP]];
        }
...
void PS5GuitarPollInputThread(PVIGEM_CLIENT client, hid_device* device, std::string devicePath, GuitarDeviceConfiguration configuration)
{
    ...
    while (IsRunning)
    {
        ...
        // we only care about the last 4 bits for the dpad
        buffer[PS5_BUF_DPAD] &= PS5_BTN_MASK_DPAD;
        // lower frets buffer matches the frets buffer
        buffer[PS5_BUF_FRETS] |= buffer[PS5_BUF_LOWER_FRETS];

        virtual_report.wButtons = ((buffer[PS5_BUF_FRETS] & PS5_BTN_MASK_FRET_1) << 12) |
                                  ((buffer[PS5_BUF_FRETS] & PS5_BTN_MASK_FRET_2) << 12) |
                                  ((buffer[PS5_BUF_FRETS] & PS5_BTN_MASK_FRET_3) << 13) |
                                  ((buffer[PS5_BUF_FRETS] & PS5_BTN_MASK_FRET_4) << 11) |
                                  ((buffer[PS5_BUF_FRETS] & PS5_BTN_MASK_FRET_5) << 4) |
                                  ((buffer[PS5_BUF_SYSTEM_BTNS] & PS5_BTN_MASK_STICK)) |
                                  ((buffer[PS5_BUF_SYSTEM_BTNS] & PS5_BTN_MASK_START) >> 1) |
                                  ((buffer[PS5_BUF_SYSTEM_BTNS] & PS5_BTN_MASK_SELECT) << 1) |
                                  ((buffer[PS5_BUF_PS_BTN] & PS5_BTN_MASK_HOME) << 10) |
                                  ((dpad_values[buffer[PS5_BUF_DPAD]]));

        virtual_report.sThumbRX = ((buffer[PS5_BUF_WHAMMY] * 255) - 32767);

        // account for deadzone
        const SHORT tilt_value = static_cast<SHORT>(min((buffer[PS5_BUF_TILT] * (128 * tilt_sensitivity)), MAXSHORT));
        virtual_report.sThumbRY = (tilt_value < tilt_deadzone ? 0 : tilt_value);

        virtual_report.sThumbLX = ((buffer[PS5_BUF_STICK_X] * 255) - 32767);
        virtual_report.sThumbLY = ((buffer[PS5_BUF_STICK_Y] * 255) - 32767);
